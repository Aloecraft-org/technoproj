...
Two things are standardised, and this covers both:

1. **The changelog** — `CHANGELOG.yaml` is written; `CHANGELOG.md` and
   `changelog.json` are generated from it and committed.
2. **The actions** — a `release.yml` that calls two shared workflows and
   keeps only its own build jobs.

Related documents, each with a narrower job:

| | |
|---|---|
| [`RELEASING.md`](RELEASING.md) | the runbook — what you type to cut a release |
| [`ALIGNMENT.md`](ALIGNMENT.md) | the version scheme, artifact naming, `BUILDINFO.txt` |
| this document | the spec — what a conforming repository contains |

The canonical copy of all three lives in
