...
def main():
    ...
    # A path, or a named profile. Named ones are looked up beside this file
    # when installed, and in status/profiles/ when run from a checkout.
    path = args.profile
    if not os.path.exists(path):
        here = os.path.dirname(os.path.abspath(__file__))
        for cand in (os.path.join(here, "profiles", args.profile + ".json"),
                     os.path.join(here, "..", "status", "profiles",
                                  args.profile + ".json")):
            if os.path.exists(cand):
                path = cand
                break
        else:
            sys.exit("collect: no profile %r (looked for a file, and for "
                     "profiles/%s.json)" % (args.profile, args.profile))
    with open(path) as f:
        profile = json.load(f)
